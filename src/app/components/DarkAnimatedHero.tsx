...
interface DarkAnimatedHeroProps {
  text: string;
  reference?: string;
  className?: string;
  ctaText?: string;
...
export const DarkAnimatedHero: React.FC<DarkAnimatedHeroProps> = ({
  text,
  reference,
  className = "",
  ctaText = "Learn More",
  ...
  const scrollToNextSlide = () => {
    if (ctaAction) {
      ...
      // Default behavior for visit page
      const nextSection = document.querySelector(".visit-info-section");
      if (nextSection) {
  ...
  return (
    <div>
      ...
      >
        <div className="max-w-6xl mx-auto px-6 pb-36 lg:pb-0">
          {reference && (
            <motion.div
              ref={referenceRef}
              className="text-white/70 font-sans text-xl w-full"
          ...
          <div ref={ctaRef} className="w-full flex justify-end">
            ...
            >
              <motion.div
                className="flex items-center px-4 py-2 bg-white text-black gap-1"
                initial={{ opacity: 0, x: -10 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.5, delay: 2, ease: "easeOut" }}
              >
                <span className="flex-1 text-lg font-sans">{ctaText}</span>
                <ArrowDown className="h-5 w-5 ml-2 group-hover:translate-y-1 transition-transform duration-200" />
              </motion.div>
            </motion.button>
          </div>
        </div>
